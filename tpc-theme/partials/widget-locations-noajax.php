<div id="locations-noajax">
 ...
 <?php if (is_page('about')) { ?>
 		<div class="five columns">
			<h5 class="tab last right quick-link"><a href="<?php echo site_url();?>/locations"> See More</a></h5>
		</div> 
<?php }?> 
		</div><!-- end heading block -->
<div class="block-inner">
	<div class="loading"><img src="<?php echo get_template_directory_uri(); ?>/imgs/loader.gif"></div>
	<div id="location-block-ajax-2">
<?php 
	$temp = $wp_query;
	$wp_query = null;
...
?>
	...
	<?php while ($wp_query->have_posts()) : $wp_query->the_post();  ?>
  		<li><a href="<?php the_permalink(); ?>"><span class="breakme-heading"><?php the_title(); ?></span></a><address><?php the_field('location_address'); ?></address>
...
<div id="navigation">
  ...
  <div class="six columns"><?php previous_posts_link('&laquo; Previous Locations') ?></div>
...
<script>
jQuery(function($) {
    $('#location-block-ajax-2').on('click', '#navigation a', function(e){
        e.preventDefault();
        var link = $(this).attr('href');
        $('#location-block-ajax-2').fadeOut(25, function(){
        	$('.loading').fadeIn(25);
            $(this).load(link + ' #location-block-ajax-2', function() {
    	  	$('.loading').fadeOut(25);
	                $(this).fadeIn(25);
            });
        });
